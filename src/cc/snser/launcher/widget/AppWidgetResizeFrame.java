...
public class AppWidgetResizeFrame  extends FrameLayout {
    ...
    public AppWidgetResizeFrame(Context context,
        ...
        mWidgetPaddingLeft = widgetView.getPaddingLeft();
        mWidgetPaddingTop = widgetView.getPaddingTop();
        mWidgetPaddingRight = widgetView.getPaddingRight();
        mWidgetPaddingBottom = widgetView.getPaddingBottom();
    ...
    private void visualizeResizeForDelta(int deltaX, int deltaY, boolean onDismiss) {
        ...
        if (mLeftBorderActive) {
            lp.leftMargin = mBaselineX + mDeltaX;
        ...
        if (mTopBorderActive) {
            lp.topMargin = mBaselineY + mDeltaY;
    ...
    void updateWidgetSizeRanges(AppWidgetHostView widgetView, Launcher launcher,
            int spanX, int spanY) {
        ...
        //将此屏上的所有数据都保存下来，因为可能是挤占了其它APP或快捷方式的位置
    ...
    static Rect getWidgetSizeRanges(Launcher launcher, int spanX, int spanY, Rect rect) {
        ...
        Workspace workspace = App.getApp().getLauncher().getWorkspace();
        CellLayout cl = (CellLayout)workspace.getChildAt(0);
        
        int width = cl.getWidth() * spanX;
        int height = cl.getHeight() * spanY;
    ...
    public void commitResize() {
        ...
        HomeItemInfo info = (HomeItemInfo)mWidgetView.getTag();
        int screen = info.screen;
        WorkspaceCellLayout layout = (WorkspaceCellLayout)mWorkspace.getChildAt(screen);
        if(layout != null){
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (tag instanceof HomeItemInfo && lp != null) {
                    HomeItemInfo itemInfo = (HomeItemInfo) tag;
                    itemInfo.cellX = lp.cellX;
                    itemInfo.cellY = lp.cellY;
                    itemInfo.spanX = lp.cellHSpan;
                    itemInfo.spanY = lp.cellVSpan;
                    DbManager.updateItemInDatabase(mLauncher,itemInfo);
                }
            }
        }
    }
    ...
    public void snapToWidget(boolean animate) {
        ...
        //mWidgetPaddingLeft = mWidgetPaddingRight = mWidgetPaddingTop = mWidgetPaddingBottom = 0;
        ...
        if (!animate) {
            ...
            //mDragLayer.updateViewLayout(this, lp);
            requestLayout();
        } else {
            PropertyValuesHolder width = PropertyValuesHolder.ofInt("width", lp.width, newWidth);
            PropertyValuesHolder height = PropertyValuesHolder.ofInt("height", lp.height,newHeight);
            PropertyValuesHolder x = PropertyValuesHolder.ofInt("x", lp.getX(), newX);
            PropertyValuesHolder y = PropertyValuesHolder.ofInt("y", lp.getY(), newY);
