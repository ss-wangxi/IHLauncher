...
public class SymmetricalLinearTween {
    private static final int MIN_RATE = 30;
    ...
    public SymmetricalLinearTween(boolean initial, int duration, TweenCallback callback) {
        this(initial, duration, callback, MIN_RATE);
    }

    public SymmetricalLinearTween(boolean initial, int duration, TweenCallback callback, float rate) {
        mValue = initial ? 1.0f : 0.0f;
        mDirection = initial;
        mDuration = duration;
        mCallback = callback;
        mHandler = new Handler();
        if (rate < MIN_RATE || rate > 1000) {
            rate = MIN_RATE;
        }
        frameTime = (int) (1000 / rate);
    }
    ...
    public void start(boolean direction, long baseTime) {
        if (direction != mDirection) {
            if (!mRunning) {
                ...
                long next = SystemClock.uptimeMillis() + frameTime;
    ...
    Runnable mTick = new Runnable() {
        public void run() {
            ...
            int frame = (int) (diff / frameTime);
            long next = base + ((frame + 1) * frameTime);
            if (diff < duration) {
