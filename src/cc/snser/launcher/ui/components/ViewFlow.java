...
//	private FlowIndicator mIndicator;
	...
	public static interface ViewSwitchListener {
		...
		void beforeSnapTo(int position);
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ViewFlow);
        mSideBuffer = a.getInt(R.styleable.ViewFlow_sidebuffer, 3);
        a.recycle();
		init();
	}
...
//		    mViewSwitchListener.beforeSnapTo(whichScreen);
		}
	...
	/**
	 * delta not more than 1
	 * */
	public void scrollToScreen(int which) {
	    if (which == mCurrentAdapterIndex) {
	        return;
	    }

	    int delta = which > mCurrentAdapterIndex ? 1 : -1;
	...
	public void computeScroll() {
		...
		if (mNextScreen != INVALID_SCREEN) {
			...
			} else if (Math.abs(getScrollX() - mScroller.getFinalX()) <= mTouchSlop / 2) {
			    int current = Math.max(0,
                        Math.min(mNextScreen, getChildCount() - 1));
			    if (mViewSwitchListener != null) {
		            mViewSwitchListener
		                    .onSwitched(null,
		                            mCurrentAdapterIndex + current - mCurrentScreen);
		        }
			}
		}
	}
...
//
//		XLog.d("viewflow", "Size of mLoadedViews: " + mLoadedViews.size() +
//				"X: " + mScroller.getCurrX() + ", Y: " + mScroller.getCurrY());
//		XLog.d("viewflow", "IndexInAdapter: " + mCurrentAdapterIndex
