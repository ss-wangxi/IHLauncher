...
public class WidgetListAdapter extends ScrollableBaseAdapter {
    ...
    final Runnable mGenerateDataCacheRunnable = new Runnable() {
        public void run() {
            if (LOGD)
                XLog.d(LOG_TAG, "mGenerateDataCacheRunnable start");
            generateDataCache();
            if (LOGD)
                XLog.d(LOG_TAG, "mGenerateDataCacheRunnable end");
        }
    };
    ...
    private void generateDataCache() {
        if (mItemMappings == null)
            return;
        XLog.d("LAUNCHER","API v1 START QUERY");
    ...
    public void bindView(ViewHolder holder, View view, Context context, int itemPosition) {
        ...
        try {
            ...
            XLog.d(LOG_TAG, "****** freeMemory = " + Runtime.getRuntime().freeMemory() / 1000
                    + " Kb");
        ...
        // if (LOGD)
        // XLog.d(LOG_TAG, "freeMemory = " + Runtime.getRuntime().freeMemory() /
        // 1000 + " Kb");

        if (Runtime.getRuntime().freeMemory() < 500000) {
            if (LOGD)
                XLog.d(LOG_TAG, "force gargabe collecting below 500kb");
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            // if (LOGD)
            // XLog.d(LOG_TAG, "*** new view");

        } else {
            holder = (ViewHolder) convertView.getTag();
            // if (LOGD)
            // XLog.d(LOG_TAG, "*** recycled view");
        }
    ...
    public void notifyToRegenerate() {
        if (LOGD)
            XLog.d(LOG_TAG, "notifyToRegenerate widgetId = " + mAppWidgetId);
    ...
    private class MyQueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            XLog.d("LAUNCHER", "API v1 QUERY COMPLETE");
            rowsElementsList.clear();
            final int size = mItemMappings.length;
