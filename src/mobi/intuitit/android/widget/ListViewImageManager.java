...
public class ListViewImageManager {
    ...
    public Drawable getImageFromUri(Context mContext, int widgetId, String imgUri) {
        ...
        if (LOGD)
            if (d != null)
                XLog.d(TAG, "image URI restored (width = " + d.getMinimumWidth() + " / weight = " + d.getMinimumHeight()
                        + ")");
        ...
        if (d == null) {
            ...
            if (ContentResolver.SCHEME_ANDROID_RESOURCE.equals(scheme)) {
                ...
                // } catch (Exception e) {
                // XLog.w("ImageView", "Unable to open content: " + mUri, e);
                // }
                XLog.w("ImageView", "Unable to open content: " + mUri);
            ...
            if (LOGD)
                XLog.d(TAG, "image URI decoded (width = " + d.getMinimumWidth() + " / weight = " + d.getMinimumHeight()
                        + ")");
    ...
    public Drawable getImageFromId(Context ctx, int widgetId, int imgId) {
        ...
        if (mCacheForImageById.containsKey(imgId) && mCacheForImageById.get(imgId) != null) {
            ...
            if (ref != null) {
                ...
                if (LOGD)
                    XLog.d(TAG, "image ID missing !!!!!!!!!!");
            }
        }

        if (LOGD)
            if (drawable != null)
                XLog.d(TAG, "image ID restored");

        if (drawable == null) {
            if (LOGD)
                XLog.d(TAG, "image ID decoded");
        ...
        if (listByUri != null) {
            for (String imgUri : listByUri) {
                if (mCacheForImageByUri.containsKey(imgUri) && mCacheForImageByUri.get(imgUri) != null) {
                    ...
                    XLog.d(TAG, "image URI removed from cache : " + imgUri);
                }
            }
        }
        ...
        if (listById != null) {
            for (Integer imgId : listById) {
                if (mCacheForImageById.containsKey(imgId) && mCacheForImageById.get(imgId) != null) {
                    ...
                    XLog.d(TAG, "image ID removed from cache : " + imgId);
                }
            }
        }
